...
app.get('/api/health', (_req, res) => {
  res.json({ ok: true, dbEnabled, host: MC_HOST, port: MC_PORT });
});
...
app.get('/api/leaderboard', async (req, res) => {
  const type = req.query.type === 'money' ? 'money' : 'playtime';
  const limit = Math.max(3, Math.min(Number(req.query.limit || 10), 50));
...
app.listen(PORT, () => {
  console.log(`Live Minecraft API server running on http://localhost:${PORT}`);
  console.log(`MC query target: ${MC_HOST}:${MC_PORT}`);
  console.log(`Database integration: ${pool ? 'enabled' : 'disabled (configure .env)'}`);
});
